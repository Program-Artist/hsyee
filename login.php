...
<div class="scroll-content padding" id="index_login">
     ...
     <input class="button button-block button-positive"  name="" type="button" class="loginbtn" value="登录"  onclick="login();"  /> 
</div>
</form>
</body>
</html>
<script>
  function login() {
     $("#login_res").html('<img src="images/loading.gif" width="24" height="24" />');
      if ($("#userName").val().replace(/[ ]/g, "") == "" || $("#userName").val().replace(/[ ]/g, "") == "请输入账号") {
          // $("#login_res").html("帳號不能為空");
           return;
      }
      if ($("#password").val().replace(/[ ]/g, "") == "") {$("#login_res").html("密码不能为空");return;}
      var s1=$("#userName").val();
      var s2='index.php?r=index/checkUser';
       $.ajax({
        type: 'get',
        url: s2,
        data: {TUNAME: s1,PASSWORD:$("#password").val(),mobile:"1",SCHOOL:"西虹市中学"},
        dataType:'json',
