...
<div class="wrapper">
    ...
    <?php  $form = $this->beginWidget('CActiveForm', get_form_login()); ?>
        <div class="item"><h1>欢迎使用中小学成绩和考评系统</h1></div>
        <div class="item">
            <?php echo $form->textField($model, 'TUNAME', array('maxlength' => 50, 'class' => 'user-input', 'placeholder'=>'账号')); ?>
        ...
        <div class="item">
        <button class="button "  type="submit"  onclick="login();" style=" color: #ff000" >登录</button></div><!--item end-->
    <?php $this->endWidget(); ?>
    </div><!--main end-->
</div><!--wrapper end-->
</body>
...
function login() {
      ...
      var s2='<?php echo $this->createUrl("index/checkUser");?>';
       $.ajax({
        type: 'get',
        url: s2,
        data: {TUNAME: s1,PASSWORD:$("#Teacher_TPWD").val(),SCHOOL:"西虹市中学"},
        dataType:'json',
